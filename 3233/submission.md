...
```text
check if the first input the same as second input depend on how many are the same the amount of money output will change
input:
the ticket and the winning number
output:
the amount of money
```
...
```text
getting  both input nd check condidition one by one 
if the winning letter is the same as ticket check for the winning number
then check if the winning letter are not the same as ticket then check the number
```
...
```text
i use my same approach by checking one by one using index
```
