...
```text
how many store open during the curtain time

input: 
 time and the amount of time to check
open and closing time
the time that used to check 
output:
the amount of store that open in that check time.
```
...
```text
step1 : get input num and check
step2 : get input start stop of each store
step3 : get the check time input
step4 : check if time >= start and < stop
step5  : cou7nt the store thst open that time
```
...
```text
i use my first approach checking the time is in between start and stop time if so count +=1
```
...
```text
check 660 should be the time that it already clos3
```
