...
```text
i want to build a bridge i have to use as much B tile as possibe and least A tile as possible but i still need the bridge to be perfect length
...
goal
Output:
amount of A tile needed or if unbuildable -1
```
...
```text
step 1: get input a,b,goal
step 2: check how many b tle use as much as possible but not more than B itself
step 3; check using what left that is A have enough
step 4: if a have enough print the usage
step 5: if not print -1
```
...
```text
i use my first approach check be first then check a if there not eniugh a  print -1
```
