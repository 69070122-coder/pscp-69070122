...
```text
get the first input as int and get another  n according to first if the input are + add 10 else if input are - SUBTRACT FROM SUM BY 5
input: number
+- for as much line as number
output: the sum up of every input
```
...
```text
step 1:  n = int(input)
step 2 : for loop n time for asking for input
step 3: in the for l;oop after get input use if to see if the input was + or -
step 4: make new variable name score  for add everything up
step 5: if input == +then add 10 to score
step 6 ; if input == - then subtract 5 off the score
step 7 ; print score 
```
...
```text
i the my first approach check if it was + or - then add or subtract from the score
```
