...
```text
it as k for 2 input the l for the length of the row and n for how many flower need to be planted then what we have to find is on the last flower that we plant what row is it on
input: l,n
output: the row of the  last flower
```
...
```text
1. get input l,n
2. from row 1 calculate how many flower can be planted
3. while sum square still less then n proceed to another row
4. if it become more than n stop and print row
```
...
```text
i use my first approach by calculate each row1 by one the input will not be over 100000 so i do not have to worry about time out
```
