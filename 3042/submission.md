...
```text
โจทย์ต้องการรับค่าจำนวนเต็ม บวกค่า หนึ่งแล้วให้ลิสจเฉพาะจำนวนที่หารด้วย 10 ลง ตั้งแต่ค่า input ถึง 0 โดยให้ output มีแถวเดียว
 
input:
รับค่าจำนวนเต็ม
Output:
ค่าที่หารด้วย 10 ลง ตั้วแต่ ค่า input - 0 
constraints:
normally the output will be printed n line
```
...
```text
step 1: รับค่า
step 2: ตรวจว่าต่านั้นหาร 10 ชงไหม
step 3; ลบด้วย 1 จากค่านั้นเพื่อเช็คตัวต่อไป
step 4: ใช้ end=" " เพื่อเปลี่ยนจากการขึ้นบรรทัดใหม่เป็นเว้นวรรค 
```
...
```text
ใช้ตามวิธีแรก ตรวจไปทีละตัวว่าหาร 10 ไม่เหลือเศษแล้วเปลี่ยน end = " "
```
