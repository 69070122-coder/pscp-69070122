...
```text
they want to kow how many time the toad needed to jump to reach it point and e4ach jump length got shorten by 2 each time if it cannot reach it print -1
input:
x,y for the destination and jump length
output:
how many time used to jump or -1
```
...
```text
get input  use while loop to add up the total if the total more or equal to y stop the loop or x become less than 0 also exit loop in everyloop x -= 2 then if total are equal or more than y print out count and if less than y print -1
...
```text
i use my first approach use4 while loop to add up the total then use if to determine if it sould print count or -1
```
