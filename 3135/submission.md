...
```text
check how many people got to investigate the package
input:
n = how many people in the circle
k = how many people from the n got to check it
t = theif
output:
how many peoplle got to investigate it
```
...
```text
first i get all the input
then i define count and current
then i use loop if current not a theif|
move to the new current do this until the current are theif
```
...
```text
i use my first approach check if the current person were theif
```
...
```text
in case of the first people was theif
```
