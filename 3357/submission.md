...
```text
first input int n for how many giraffe that we going to input and the the tallest one compare to both side get 1 count 
input:
how many giraffe  and that much line of how tall are each giraffe
output:
the count of giraffe that tallest compare to both side of it
```
...
```text
i going to check index 1 by one start with the 0 and compare it to the next one if it was taller count += 1if not continue
```
...
```text
i have a problem with only 1 giraffe input it have noone to compare so the output is 0 so i change tactic when it start it will be only 1 to compare so if the input was 1 or the edge was  highest the input would += 1 then the rest is pretty much the same as the first approach
```
...
```text
case that two of the same input
```
