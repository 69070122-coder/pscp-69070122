...
```text
the task is for me to get 2 input primary color and mix it to make another color buut if the input wasn't a prime color print Error instead 
```
...
```text
Step 1:get the primary color input
Step 2: check in if statement
...
```text
i do it as my step but i found out 2 of the testcase are incorrect so i guest it must have been a same primary color so i fix the if statment and i was correct
```
...
```text
i think lower case word should not be able to create another as well
```
...
```text
just 2 of the same primary colors
```
