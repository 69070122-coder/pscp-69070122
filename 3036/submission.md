...
```text
they want me to write a code to calculate how many time they have to break a wall to get from the input point

input:
N((1 <= N <= 100,000)

output:
amount of time that they have to break the wall
```
...
```text
step 1 : get input
step 2 : find the row which i use 10 min for it
step 3 : find the location
step 4 : find out that it a point up triangle or point down
step 5: if point up calculate from the 2(row-1) i use 20 min to figure it out
step 6 : if it a point down then it should use 1 less move then the point up
```
...
```text
i use the same approach i check it it point up or point down by %2 if it an odd number it will be = 1 and that mean it have to been calculate by point up method if not then i just - 1
```
