...
```text
module for calculate how much you have to pay from where it from where it go and how much it weight
 
input:
input start , destination
input weight
Output:
amount of money i have to pay with 2 decimal
constraints:
none
```
...
```text
step 1: get input
step 2: if both first and destination input meet what was set print how much would it take
step 3; if one of which does not print error
...
```text
i use my first approach by checking if both input meet the condition print the price in 2 decimal if not print error
```
...
```text
lowercase which should be an error
```
