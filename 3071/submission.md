...
```text
get find out how many number that can be devide by D and have R left
...
input R
Output:
a count of how many number that can be devided by D and have R left that are in the range of A to B
constraints:
none
```
...
```text
step 1: get input A,B,D,R
step 2: check if A can % D and equal to R if so += 1 to count
step 3; add 1 to a to check another number
step 4: put in while loop to make it keep going until a = b
step 5: print count
```
...
```text
ใช้ตามวิธีแรก ตรวจไปทีละตัวว่าหาร 1D แล้ว เหลือเศษ R ไหม
```
